...
(defcustom disaster-objdump "/usr/local/Cellar/binutils/2.23.2/bin/gobjdump -d -M att -Sl --no-show-raw-insn"
  "The command name and flags for running objdump."
  :group 'disaster
  :type 'string)

(defcustom disaster-otool "otool -tV -X -q"
  "The command name and flags for running otool."
  :group 'disaster
  :type 'string)
...
(defcustom disaster-buffer-assembly "*assembly*"
  "Buffer name to use for assembly output."
  :group 'disaster
  :type 'string)
...
- Run objdump/otool inside a new window while maintaining focus.
             ...
             (cc (if make-root
                   ...
                   (if (string-match "\\.c[cp]p?$" file)
                     ...
                     (format "%s %s -g -c -o %s %s"
                             ...
                             obj-file file))))
             (dump (format "%s %s" disaster-otool obj-file))
