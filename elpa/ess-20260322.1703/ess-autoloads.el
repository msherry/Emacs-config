...
In addition to any hooks its parent mode might have run, this mode runs the
hook `ess-gretl-mode-hook', as the final or penultimate step during
initialization.
...
In addition to any hooks its parent mode might have run, this mode runs the
hook `ess-r-mode-hook', as the final or penultimate step during
initialization.
...
this mode runs the hook `ess-r-transcript-mode-hook', as the final or
...
In addition to any hooks its parent mode might have run, this mode runs the
hook `SAS-mode-hook', as the final or penultimate step during initialization.
